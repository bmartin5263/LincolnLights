...
public:
  ...
  virtual auto calculateColor(const RpmGaugeCalculations& calculations, const RpmScene& scene) const -> rgb::Color = 0;
  ...
  friend class RpmScene;
};

class PartitionedRpmColorMode : public RpmColorMode {
public:
  auto calculateColor(const RpmGaugeCalculations& calculations, const RpmScene& scene) const -> rgb::Color override;
